package com.infoevent.joparis.user.controllers;
...
import com.infoevent.joparis.user.entities.Role;
import com.infoevent.joparis.user.utils.UserUtils;
...
public class UserController {
    ...
    public ResponseEntity<User> createUser(@Valid @RequestBody User user) {
        ...
        String key = keyGeneratorService.generateSecureRandomKey();
        user.setKey(key);

        User createdUser = userService.createUser(user);
        sendTicketNotification(createdUser);
    ...
    private void sendTicketNotification(User user) {
        log.info("Attempting to send email to {}", user.getEmail());
        NotificationRequest notificationRequest = UserUtils.buildNotificationRequest(user);
        String subject = "Bienvenue sur la billeterie des J.0 2024 !";
        String body = NotificationUtils.constructWelcomeEmailBody(notificationRequest);
        notificationService.sendEmail(notificationRequest.getUser().getEmail(), subject, body);
    }
}
